...
包裹、中英文标点、Unicode 兼容字符、符号或纯数字列表标记，或仅用这些词拼成
组合，仍视为泛化假值，不能代替可复核的完整叙述。Markdown task marker、Unicode
变体选择符、组合标记和零宽/格式控制字符也不构成语义内容；隐藏字符不能把
`PASSDONE`、`PASSOK` 等泛化词拼接伪装成说明。含数字或量纲的真实说明不会仅因
数字被忽略而失败；只有去掉纯数字列表词元后，剩余字母数字语义流可以完全分解为
上述泛化词时才拒绝。
