  ...
  public:
    explicit TemporaryInput(const std::string& contents) {
        ...
        path_ =
            ...
            ("csc3-inp-case-" + std::to_string(tick) + "-" + std::to_string(sequence++) + ".inp");
...
void require_parse_failure(const std::string& contents, const std::string& expected_fragment,
                           Fn&& operation) {
...
void require_finite_symmetric_segments(const AssemblyCase& assembly_case) {
    ...
    require_equal(offsets.size(), assembly_case.element_dof_map.element_ids.size() + 1,
                  "element matrix offset count");
    for (std::size_t element = 0; element < assembly_case.element_dof_map.element_ids.size();
         ++element) {
        ...
        const std::size_t dimension = assembly_case.element_type == ElementType::Tet4 ? 12 : 24;
        ...
        for (std::size_t row = 0; row < dimension; ++row) {
            for (std::size_t column = 0; column < dimension; ++column) {
                ...
                require_true(std::isfinite(value), "local matrix contains a nonfinite value");
...
void test_c3d4_gapped_nodes_and_unsorted_elements() {
    ...
    require_equal(parsed.external_element_ids, std::vector<ElementId>({42, 7}),
                  "input element order");
    require_equal(parsed.element_node_offsets, std::vector<Offset>({0, 4, 8}),
                  "element node offsets");
    require_equal(parsed.compact_node_indices, std::vector<std::size_t>({1, 3, 0, 2, 1, 0, 2, 3}),
                  "compact node mapping");

    AssemblyCase assembly_case = make_assembly_case(parsed);
    require_equal(assembly_case.element_dof_map.element_ids, std::vector<ElementId>({7, 42}),
                  "canonical external element IDs");
    require_equal(assembly_case.element_dof_map.element_dof_offsets,
                  std::vector<Offset>({0, 12, 24}), "Tet4 DOF offsets");
    require_equal(assembly_case.element_dof_map.global_dof_indices[0], GlobalDofIndex{3},
                  "canonical first element first DOF");
    require_equal(assembly_case.element_dof_map.global_dof_indices[3], GlobalDofIndex{0},
...
void test_c3d8_case_insensitive_physical_matrix() {
    ...
    require_equal(loaded.element_dof_map.element_ids, std::vector<ElementId>({9}),
...
void test_rejected_inputs_report_lines() {
    ...
    const std::string nodes = "*Node\n1,0,0,0\n2,1,0,0\n3,0,1,0\n4,0,0,1\n";
    require_parse_failure("*Node\n1,0,0,0\n1,1,0,0\n*Element,type=C3D4\n1,1,1,1,1\n",
                          "duplicate node",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure(nodes + "*Element,type=C3D4\n8,1,2,3,4\n8,1,2,3,4\n", "duplicate element",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure(nodes + "*Element,type=C3D4\n8,1,2,3,999\n", "unknown node",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure("*Node\n1,not-a-number,0,0\n*Element,type=C3D4\n1,1,1,1,1\n",
    ...
    require_parse_failure(nodes + "*Element,type=C3D4\n8,1,2,3\n", "element record",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure(nodes + "*Element,type=C3D10\n", "unsupported element type",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure(nodes + "*Element,type=C3D4\n1,1,2,3,4\n*Element,type=C3D8\n",
                          "mixed element formulations",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure("*Heading\n** no mesh\n", "empty mesh",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
    require_parse_failure(nodes + "*Element,type=C3D4\n2147483648,1,2,3,4\n", "element identifier",
                          [](const auto& path) { static_cast<void>(parse_abaqus_inp(path)); });
}
