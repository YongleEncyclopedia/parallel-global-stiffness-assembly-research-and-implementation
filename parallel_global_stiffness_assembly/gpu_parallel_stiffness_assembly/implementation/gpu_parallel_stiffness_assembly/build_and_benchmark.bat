...
echo ========================================================
echo   GPU Parallel Stiffness Assembly - Build and Benchmark
echo ========================================================
echo.
...
echo.
echo [3/4] Building warp aggregation smoke test...
cd /d "%~dp0"

REM Compile the standalone CUDA smoke test.
nvcc -O2 -arch=sm_120 -allow-unsupported-compiler -o warp_aggregation_smoke_test.exe tests\cuda\warp_aggregation_smoke_test.cu
if %ERRORLEVEL% neq 0 (
    nvcc -O2 -arch=sm_86 -allow-unsupported-compiler -o warp_aggregation_smoke_test.exe tests\cuda\warp_aggregation_smoke_test.cu
    if %ERRORLEVEL% neq 0 (
        echo WARNING: warp aggregation smoke test compilation failed
    )
)

if exist warp_aggregation_smoke_test.exe (
    echo.
    echo Running warp aggregation smoke test...
    warp_aggregation_smoke_test.exe
    echo.
)
