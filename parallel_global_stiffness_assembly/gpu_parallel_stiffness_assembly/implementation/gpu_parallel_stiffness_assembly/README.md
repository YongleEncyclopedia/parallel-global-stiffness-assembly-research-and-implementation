...
3. **工作队列** (`Work_Queue`) - 动态负载均衡
4. **CPU串行** (`CPU_Serial`) - 基准参考

> 注：`Prefix_Scan` 的实验性实现仍保留在源码中，但当前基准结果默认不纳入该算法。
...
```bash
# 生成所有标准测试网格
python scripts/generate_structured_mesh.py --all

# 生成指定规模的网格
python scripts/generate_structured_mesh.py --type hex8 --dof 100000 -o data/meshes/hex8_100k.mesh
```
