...
结果写入 `build/example-results/<时间戳>/`，其中包括逐样本 CSV、汇总 JSON、运行记录和中文 `summary.md`。只有完整 Git 仓库中的 Windows 流程保留原正式证据 schema；自包含包和 Linux 结果记录 `formal_evidence=false`。

## CPU 与内存读数

任务管理器中的 CPU 只在候选并行组装阶段明显升高；输入准备、独立串行参考和正确性检查不会持续占满全部核心，所以整个子进程的平均占用通常不高。

Windows 使用 `GetProcessMemoryInfo.PeakWorkingSetSize` 报告进程峰值工作集；Linux 使用 `wait4(...).ru_maxrss` 报告进程峰值常驻集。两个数字都覆盖整个 benchmark 子进程，但操作系统定义不同，不能作为完全相同的跨平台指标。`estimated_persistent_bytes` 只表示算法所拥有持久向量的容量估计，不是常驻集，也不是算法峰值内存。
