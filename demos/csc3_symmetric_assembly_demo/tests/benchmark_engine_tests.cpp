...
std::size_t expected_payload_bytes(const AssemblyCase& assembly_case,
                                   int thread_count) {
    SymmetricCscAssembler assembler;
    assembler.build_symbolic_parallel(assembly_case.element_dof_map,
                                      thread_count);
    ...
    return matrix.column_offsets.size() * sizeof(Offset) +
           ...
           plan.element_ids.size() * sizeof(ElementId) +
...
void require_statistics_finite(const SummaryStatistics& statistics,
...
void require_successful_result(const BenchmarkResult& result,
    ...
    require_equal(result.configuration.benchmark_case,
                  expected_case,
                  "benchmark case");
    ...
    require_true(result.correctness.structure_matches,
                 "serial and candidate structures differ");
    require_true(result.correctness.status == "PASS",
                 "matrix correctness did not pass");
    ...
    require_true(result.correctness.max_absolute_error <=
                     result.correctness.max_absolute_tolerance,
                 "maximum absolute threshold exceeded");
    require_equal(result.performance_gate_status,
                  std::string("NOT_APPLICABLE_GENERATED_CASE"),
                  "generated performance gate status");
    require_equal(result.performance_evidence_level,
                  std::string("ci-smoke"),
                  "performance evidence level");

    const std::size_t repeat_count =
        static_cast<std::size_t>(result.configuration.repeat_count);
    require_statistics_finite(result.serial_measured.symbolic_total_ms,
                              repeat_count,
                              "serial symbolic");
    require_statistics_finite(result.serial_measured.numeric_total_ms,
                              repeat_count,
                              "serial numeric");
    require_equal(result.per_thread_measured.size(),
    ...
    for (std::size_t thread_ordinal = 0;
        ...
        require_statistics_finite(summary.symbolic_pattern_ms,
                                  repeat_count,
                                  "symbolic pattern");
        require_statistics_finite(summary.symbolic_scatter_ms,
                                  repeat_count,
                                  "symbolic scatter");
        require_statistics_finite(summary.symbolic_total_ms,
                                  repeat_count,
                                  "symbolic total");
        require_statistics_finite(summary.numeric_reset_ms,
                                  repeat_count,
                                  "numeric reset");
        ...
        require_statistics_finite(summary.numeric_total_ms,
                                  repeat_count,
                                  "numeric total");
        require_statistics_finite(summary.amortized_total_ms,
                                  repeat_count,
                                  "amortized total");
        ...
        for (std::size_t row_offset = 0;
            ...
            require_true(sample.candidate_timings.symbolic_total_ms +
                                 kClockToleranceMs >=
            ...
            require_true(sample.candidate_timings.numeric_total_ms +
                                 kClockToleranceMs >=
...
void test_known_statistics_and_validation() {
    const SummaryStatistics statistics =
        summarize_measured_values({1.0, 2.0, 3.0, 4.0});
    ...
    require_close(statistics.population_standard_deviation_ms,
                  std::sqrt(1.25),
                  1.0e-15,
    ...
    require_close(statistics.coefficient_of_variation,
                  std::sqrt(1.25) / 2.5,
                  1.0e-15,
                  "statistics coefficient of variation");

    const SummaryStatistics zeros = summarize_measured_values({0.0, 0.0});
    require_close(zeros.coefficient_of_variation,
                  0.0,
                  0.0,
                  "zero coefficient of variation");
    require_throws<std::invalid_argument>(
        [] { static_cast<void>(summarize_measured_values({})); },
...
void test_generated_tet4_and_hex8_benchmarks() {
    ...
    BenchmarkConfiguration hex8_configuration =
        small_configuration(BenchmarkCase::GeneratedHex8);
...
void test_production_header_has_no_serial_or_benchmark_api() {
    ...
    require_true(class_begin != std::string::npos &&
                     public_begin != std::string::npos &&
                     private_begin != std::string::npos,
                 "could not locate assembler public section");
    std::string public_section =
        contents.substr(public_begin, private_begin - public_begin);
    std::transform(public_section.begin(),
                   public_section.end(),
                   public_section.begin(),
