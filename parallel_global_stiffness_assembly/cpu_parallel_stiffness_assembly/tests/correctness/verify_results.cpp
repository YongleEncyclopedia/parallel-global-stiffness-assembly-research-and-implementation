...
void verify_algorithms(const Mesh& mesh, StiffnessModel stiffness_model, int threads) {
    CsrMatrix csr = CsrMatrix::build_sparsity(mesh);
    AssemblyPlan plan = build_assembly_plan(mesh, csr);

    AssemblyOptions options;
    options.threads = threads;
    options.stiffness_model = stiffness_model;
...
int main() {
    try {
        ...
        verify_algorithms(generated_mesh, StiffnessModel::LinearElasticSolid, 4);
        ...
        verify_algorithms(gap_labels_mesh, StiffnessModel::PhysicsTet4, 3);
        return 0;
