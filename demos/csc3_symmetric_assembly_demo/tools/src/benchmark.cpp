...
AssemblyPlan canonicalize_topology(const ElementDofMap& topology,
    ...
    std::sort(ordinals.begin(), ordinals.end(), [&topology](std::size_t left,
                                                            std::size_t right) {
        return topology.element_ids[left] < topology.element_ids[right];
    });
    for (std::size_t index = 1; index < ordinals.size(); ++index) {
        if (topology.element_ids[ordinals[index - 1]] ==
            topology.element_ids[ordinals[index]]) {
    ...
    for (const std::size_t ordinal : ordinals) {
        const std::size_t begin = offset_to_size(
            topology.element_dof_offsets[ordinal], "element DOF offset");
        const std::size_t end = offset_to_size(
            topology.element_dof_offsets[ordinal + 1], "element DOF offset");
        if (begin == end) {
        ...
        if (std::adjacent_find(local_dofs.begin(), local_dofs.end()) !=
            local_dofs.end()) {
        ...
        plan.global_dof_indices.insert(plan.global_dof_indices.end(),
                                       topology.global_dof_indices.begin() +
                                           static_cast<std::ptrdiff_t>(begin),
                                       topology.global_dof_indices.begin() +
                                           static_cast<std::ptrdiff_t>(end));
        plan.element_dof_offsets.push_back(size_to_offset(
            plan.global_dof_indices.size(), "canonical global DOF array size"));
    }
    if (maximum_dof < 0) {
    ...
    if (std::find(observed_dofs.begin(), observed_dofs.end(), false) !=
        observed_dofs.end()) {
        throw std::invalid_argument(
...
SerialSymbolicState build_serial_symbolic(const ElementDofMap& topology) {
    ...
    const std::size_t dimension =
        static_cast<std::size_t>(result.matrix.dimension);
    ...
    for (std::size_t element = 0; element < element_count; ++element) {
        const std::size_t begin = offset_to_size(
            result.plan.element_dof_offsets[element], "canonical element DOF offset");
        const std::size_t end = offset_to_size(
            result.plan.element_dof_offsets[element + 1], "canonical element DOF offset");
        ...
        const std::size_t triangular_count = checked_multiply(
            ...
            2;
        result.plan.element_scatter_offsets[element + 1] = size_to_offset(
            checked_add(offset_to_size(result.plan.element_scatter_offsets[element],
                                       "scatter offset"),
                        triangular_count,
                        "total scatter count"),
            "total scatter count");
        for (std::size_t local_row = begin; local_row < end; ++local_row) {
            for (std::size_t local_column = local_row; local_column < end;
                 ++local_column) {
                const GlobalDofIndex first =
                    result.plan.global_dof_indices[local_row];
                const GlobalDofIndex second =
                    result.plan.global_dof_indices[local_column];
    ...
    for (std::size_t column = 0; column < dimension; ++column) {
        ...
        result.matrix.column_offsets[column + 1] = size_to_offset(
            checked_add(offset_to_size(result.matrix.column_offsets[column],
                                       "column offset"),
                        rows.size(),
                        "CSC3 nonzero count"),
            "CSC3 nonzero count");
    }
    const std::size_t nonzero_count = offset_to_size(
        result.matrix.column_offsets.back(), "CSC3 nonzero count");
    result.matrix.row_indices.resize(nonzero_count);
    result.matrix.values.assign(nonzero_count, 0.0);
    for (std::size_t column = 0; column < dimension; ++column) {
        const std::size_t begin = offset_to_size(
            result.matrix.column_offsets[column], "column offset");
        std::copy(column_rows[column].begin(),
                  column_rows[column].end(),
                  result.matrix.row_indices.begin() +
                      static_cast<std::ptrdiff_t>(begin));
    }

    const std::size_t scatter_count = offset_to_size(
        result.plan.element_scatter_offsets.back(), "total scatter count");
    result.plan.scatter_indices.assign(scatter_count, 0);
    for (std::size_t element = 0; element < element_count; ++element) {
        const std::size_t dof_begin = offset_to_size(
            result.plan.element_dof_offsets[element], "canonical element DOF offset");
        const std::size_t dof_end = offset_to_size(
            result.plan.element_dof_offsets[element + 1], "canonical element DOF offset");
        std::size_t scatter_position = offset_to_size(
            result.plan.element_scatter_offsets[element], "scatter offset");
        for (std::size_t local_row = dof_begin; local_row < dof_end; ++local_row) {
            for (std::size_t local_column = local_row; local_column < dof_end;
                ...
                const std::size_t column_begin = offset_to_size(
                    result.matrix.column_offsets[column_index], "column offset");
                const std::size_t column_end = offset_to_size(
                    result.matrix.column_offsets[column_index + 1], "column offset");
                const auto begin = result.matrix.row_indices.begin() +
                                   static_cast<std::ptrdiff_t>(column_begin);
                const auto end = result.matrix.row_indices.begin() +
                ...
                result.plan.scatter_indices[scatter_position++] = size_to_offset(
                    column_begin + static_cast<std::size_t>(found - begin),
                    "scatter target");
            }
        }
    }
...
SerialNumericKernelPlan prepare_serial_numeric_kernel(
    ...
    for (std::size_t element = 0; element < element_count; ++element) {
        ...
        const std::size_t dof_end = offset_to_size(
            plan.element_dof_offsets[element + 1], "element DOF offset");
        ...
        const std::size_t value_end = offset_to_size(
            element_matrices.element_value_offsets[element + 1],
            "element matrix value offset");
...
BenchmarkCorrectness compare_sparse(const Csc3Matrix& candidate,
...
std::size_t vector_payload_bytes(std::size_t count,
...
std::size_t estimated_persistent_bytes(const SymmetricCscAssembler& assembler) {
    ...
    std::size_t total = 0;
    const auto add_payload = [&total](std::size_t count,
                                      std::size_t element_size,
                                      const char* label) {
        total = checked_add(total,
                            vector_payload_bytes(count, element_size, label),
    ...
    add_payload(plan.element_dof_offsets.size(), sizeof(Offset),
                "element_dof_offsets bytes");
    add_payload(plan.global_dof_indices.size(), sizeof(GlobalDofIndex),
                "global_dof_indices bytes");
...
void merge_correctness(BenchmarkCorrectness& aggregate,
                       const BenchmarkCorrectness& current) {
    aggregate.structure_matches =
        aggregate.structure_matches && current.structure_matches;
    aggregate.relative_frobenius_error = std::max(
...
SummaryStatistics summarize_measured_values(const std::vector<double>& values) {
    ...
    for (const double value : values) {
        if (!std::isfinite(value) || value < 0.0) {
            throw std::invalid_argument(
                "statistics values must be finite and nonnegative");
        }
    }
    ...
    const long double variance =
        squared_difference_sum / static_cast<long double>(values.size());
    ...
    result.population_standard_deviation_ms =
    ...
    if (result.mean_ms == 0.0) {
        ...
        result.coefficient_of_variation =
            result.population_standard_deviation_ms / result.mean_ms;
    }
...
BenchmarkResult run_generated_benchmark(
    ...
    numeric_reference_assembler.build_symbolic_parallel(
        assembly_case.element_dof_map, 1);
    ...
    result.dof_count =
        static_cast<std::size_t>(numeric_reference_assembler.matrix().dimension);
    ...
    for (const int thread_count : configuration.thread_counts) {
        ...
        for (std::size_t sample_index = 0; sample_index < total_sample_count;
            ...
            symbolic_timings.push_back(
                BenchmarkAccess::timings(symbolic_assembler));
        }

        SymmetricCscAssembler numeric_assembler;
        numeric_assembler.build_symbolic_parallel(assembly_case.element_dof_map,
                                                  thread_count);
        if (!BenchmarkAccess::symbolic_used_requested_team_in_all_regions(
                numeric_assembler)) {
            throw std::runtime_error(
                "OpenMP did not provide the requested team for numeric setup");
        }
        ...
        summary.symbolic_speedup = positive_speedup(
        ...
        for (std::size_t sample_index = 0; sample_index < total_sample_count;
            ...
            sample.candidate_timings.numeric_reset_ms =
                numeric_timings[index].numeric_reset_ms;
            sample.candidate_timings.numeric_kernel_ms =
                numeric_timings[index].numeric_kernel_ms;
            sample.candidate_timings.numeric_total_ms =
                numeric_timings[index].numeric_total_ms;
