...
void require_finite_nonnegative(double value, const char* label) {
    if (!std::isfinite(value) || value < 0.0) {
        throw std::runtime_error(std::string(label) + " must be finite and nonnegative");
    }
}
...
void validate_cli_configuration(const BenchmarkConfiguration& configuration) {
    const std::string evidence = evidence_level_name(configuration.performance_evidence_level);
    ...
    for (const int thread_count : configuration.thread_counts) {
        if (thread_count <= 0) {
            throw std::invalid_argument("threads list must contain only positive values");
        }
...
void validate_statistics(const SummaryStatistics& statistics, std::size_t expected_count,
                         const char* label) {
    if (statistics.sample_count != expected_count) {
        throw std::runtime_error(std::string(label) + " has an unexpected sample count");
    }
    require_finite_nonnegative(statistics.mean_ms, label);
    require_finite_nonnegative(statistics.median_ms, label);
    require_finite_nonnegative(statistics.population_standard_deviation_ms, label);
    ...
    if (statistics.minimum_ms > statistics.median_ms ||
        ...
        throw std::runtime_error(std::string(label) + " has inconsistent order statistics");
    }
}
...
void require_scale_aware_equal(double actual, double expected, const char* label) {
    if (!std::isfinite(actual) || !std::isfinite(expected) ||
        !scale_aware_equal(actual, expected)) {
        throw std::runtime_error(std::string(label) + " disagrees with measured raw samples");
    }
}

template <typename MatrixEvidence>
void validate_reference_scaled_tolerance(const MatrixEvidence& matrix, const char* label) {
    require_finite_nonnegative(matrix.reference_max_absolute_value, label);
    const double expected = kMaximumAbsoluteBaseTolerance +
                            kMaximumAbsoluteScaleTolerance * matrix.reference_max_absolute_value;
...
void require_statistics_match(const SummaryStatistics& actual, const SummaryStatistics& expected,
                              const char* label) {
    ...
    require_scale_aware_equal(actual.population_standard_deviation_ms,
                              expected.population_standard_deviation_ms, label);
    ...
    require_scale_aware_equal(actual.coefficient_of_variation, expected.coefficient_of_variation,
                              label);
}

double recomputed_speedup(double serial_median, double candidate_median, const char* label) {
...
void validate_candidate_timings(const CandidateTimings& timings) {
    require_finite_nonnegative(timings.symbolic_pattern_ms, "symbolic_pattern_ms");
    require_finite_nonnegative(timings.symbolic_scatter_ms, "symbolic_scatter_ms");
    require_finite_nonnegative(timings.symbolic_total_ms, "symbolic_total_ms");
    require_finite_nonnegative(timings.numeric_reset_ms, "numeric_reset_ms");
    require_finite_nonnegative(timings.numeric_kernel_ms, "numeric_kernel_ms");
    require_finite_nonnegative(timings.numeric_total_ms, "numeric_total_ms");
...
void validate_validation_cases(const BenchmarkResult& result) {
    if (result.validation_cases.size() != 2) {
        throw std::runtime_error("validation evidence must contain exactly Tet4 and Hex8 cases");
    }
    ...
    for (std::size_t index = 0; index < result.validation_cases.size(); ++index) {
        ...
        const std::string expected_case_name = index == 0 ? "cube_tet4_1x1x1" : "cube_hex8_1x1x1";
        ...
        if (validation.case_name != expected_case_name ||
            validation.element_type != expected_element_type || validation.node_count != 8 ||
            validation.element_count != expected_element_count || validation.dof_count != 24) {
            throw std::runtime_error("validation evidence fixture identity or size is invalid");
        }
        if (validation.thread_count != expected_thread_count) {
            throw std::runtime_error("validation evidence thread count is inconsistent");
        }
        ...
        require_finite_nonnegative(displacement.relative_displacement_error,
                                   "validation relative_displacement_error");
        require_finite_nonnegative(displacement.parallel_relative_residual,
                                   "validation parallel_relative_residual");
        require_finite_nonnegative(displacement.serial_relative_residual,
...
void validate_result(const BenchmarkResult& result) {
    ...
    if (result.node_count == 0 || result.element_count == 0 || result.dof_count == 0 ||
    ...
    if (result.performance_evidence_level != expected_evidence) {
        throw std::runtime_error("performance evidence level disagrees with configuration");
    }
    ...
    if (result.per_thread_measured.size() != result.configuration.thread_counts.size()) {
    ...
    for (std::size_t index = 0; index < result.per_thread_measured.size(); ++index) {
        ...
        validate_statistics(summary.symbolic_pattern_ms, repeat_count,
                            "symbolic pattern statistics");
        validate_statistics(summary.symbolic_scatter_ms, repeat_count,
                            "symbolic scatter statistics");
        validate_statistics(summary.symbolic_total_ms, repeat_count, "symbolic total statistics");
        validate_statistics(summary.numeric_reset_ms, repeat_count, "numeric reset statistics");
        validate_statistics(summary.numeric_kernel_ms, repeat_count, "numeric kernel statistics");
        validate_statistics(summary.numeric_algorithm_ms, repeat_count,
                            "numeric algorithm statistics");
        validate_statistics(summary.numeric_total_ms, repeat_count, "numeric total statistics");
        validate_statistics(summary.amortized_total_ms, repeat_count, "amortized total statistics");
        require_finite_nonnegative(summary.symbolic_speedup, "symbolic_speedup");
    ...
    const std::size_t expected_samples = checked_multiply(
        samples_per_thread, result.configuration.thread_counts.size(), "benchmark sample count");
    ...
    for (std::size_t thread_ordinal = 0; thread_ordinal < result.configuration.thread_counts.size();
        ...
        for (std::size_t sample_ordinal = 0; sample_ordinal < samples_per_thread;
            ...
            const BenchmarkSample& sample =
                result.samples[thread_ordinal * samples_per_thread + sample_ordinal];
            if (sample.thread_count != result.configuration.thread_counts[thread_ordinal] ||
            ...
            const SampleKind expected_kind =
                sample_ordinal < warmup_count ? SampleKind::Warmup : SampleKind::Measured;
            ...
            require_finite_nonnegative(sample.amortized_total_ms, "sample amortized_total_ms");
            require_finite_nonnegative(sample.symbolic_speedup, "sample symbolic_speedup");
            require_finite_nonnegative(sample.numeric_speedup, "sample numeric_speedup");
            ...
            const double amortized_tolerance = 1.0e-12 * std::max(1.0, expected_amortized);
            if (std::abs(sample.amortized_total_ms - expected_amortized) > amortized_tolerance) {
                throw std::runtime_error("sample amortized timing is inconsistent");
            }
    ...
    for (std::size_t thread_ordinal = 0; thread_ordinal < result.configuration.thread_counts.size();
        ...
        for (std::size_t sample_ordinal = warmup_count; sample_ordinal < samples_per_thread;
             ++sample_ordinal) {
            const BenchmarkSample& sample =
                result.samples[thread_ordinal * samples_per_thread + sample_ordinal];
            ...
            const double numeric_algorithm_ms =
                timings.numeric_reset_ms + timings.numeric_kernel_ms;
            require_finite_nonnegative(numeric_algorithm_ms, "numeric reset plus kernel timing");
        ...
        const ThreadBenchmarkSummary& actual = result.per_thread_measured[thread_ordinal];
        ...
        const SummaryStatistics expected_symbolic_total = summarize_measured_values(symbolic_total);
        const SummaryStatistics expected_numeric_reset = summarize_measured_values(numeric_reset);
        const SummaryStatistics expected_numeric_kernel = summarize_measured_values(numeric_kernel);
        const SummaryStatistics expected_numeric_algorithm =
            summarize_measured_values(numeric_algorithm);
        const SummaryStatistics expected_numeric_total = summarize_measured_values(numeric_total);
        const SummaryStatistics expected_amortized_total =
            summarize_measured_values(amortized_total);
        require_statistics_match(actual.symbolic_pattern_ms, expected_symbolic_pattern,
                                 "symbolic pattern statistics");
        require_statistics_match(actual.symbolic_scatter_ms, expected_symbolic_scatter,
                                 "symbolic scatter statistics");
        require_statistics_match(actual.symbolic_total_ms, expected_symbolic_total,
                                 "symbolic total statistics");
        require_statistics_match(actual.numeric_reset_ms, expected_numeric_reset,
                                 "numeric reset statistics");
        require_statistics_match(actual.numeric_kernel_ms, expected_numeric_kernel,
                                 "numeric kernel statistics");
        require_statistics_match(actual.numeric_algorithm_ms, expected_numeric_algorithm,
                                 "numeric algorithm statistics");
        require_statistics_match(actual.numeric_total_ms, expected_numeric_total,
                                 "numeric total statistics");
        require_statistics_match(actual.amortized_total_ms, expected_amortized_total,
                                 "amortized total statistics");

        const double expected_symbolic_speedup =
            recomputed_speedup(expected_serial_symbolic.median_ms,
                               expected_symbolic_total.median_ms, "symbolic speedup");
        const double expected_numeric_speedup =
            recomputed_speedup(expected_serial_numeric.median_ms,
                               expected_numeric_algorithm.median_ms, "numeric speedup");
        require_scale_aware_equal(actual.symbolic_speedup, expected_symbolic_speedup,
                                  "symbolic speedup");
    ...
    if (result.performance_gate_status != expected_gate.status ||
        ...
        actual_gate.symbolic_speedup_threshold != expected_gate.symbolic_speedup_threshold ||
        actual_gate.maximum_coefficient_of_variation !=
            expected_gate.maximum_coefficient_of_variation) {
        throw std::runtime_error("performance gate disagrees with measured thread statistics");
    }
}
...
void append_statistics_json(std::ostream& output, const SummaryStatistics& statistics,
    ...
    output << "{\n"
           ...
           << indent << "  \"coefficient_of_variation\": " << statistics.coefficient_of_variation
           << '\n'
           << indent << '}';
}
...
void validate_output_path_parent(const std::filesystem::path& path) {
    ...
    if (!parent.empty()) {
        const bool parent_is_directory = std::filesystem::is_directory(parent, error);
        if (error) {
            throw std::runtime_error("could not inspect output parent: " + error.message());
        }
...
void validate_materialized_input_path(const std::filesystem::path& path) {
    ...
    if (first_line == "version https://git-lfs.github.com/spec/v1" ||
        ...
        throw std::invalid_argument("Git LFS pointer is not a materialized WindHub input");
    }
}
...
void write_new_file(const std::filesystem::path& path, const std::string& contents) {
    ...
    while (offset < contents.size()) {
        const DWORD requested =
            static_cast<DWORD>(std::min(kWriteChunkBytes, contents.size() - offset));
        DWORD written = 0;
        if (!WriteFile(handle, contents.data() + offset, requested, &written, nullptr) ||
            written == 0) {
    ...
    if (failure != ERROR_SUCCESS) {
        ...
        throw std::runtime_error(
            ...
            std::error_code(static_cast<int>(failure), std::system_category()).message());
    }
#else
    const int descriptor = ::open(path.c_str(), O_WRONLY | O_CREAT | O_EXCL, 0666);
    if (descriptor < 0) {
        const int error = errno;
        throw std::runtime_error("could not exclusively create output file: " + path.string() +
    ...
    std::size_t offset = 0;
    while (offset < contents.size()) {
        const std::size_t requested = std::min(kWriteChunkBytes, contents.size() - offset);
        const ssize_t written = ::write(descriptor, contents.data() + offset, requested);
        if (written < 0) {
            if (errno == EINTR) {
                continue;
            }
    ...
    if (failure != 0) {
        ...
        throw std::runtime_error("failed while writing or closing output file: " + path.string() +
                                 ": " +
...
int parse_integer(const std::string& text, const char* option) {
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != end) {
        throw std::invalid_argument(std::string(option) + " requires a base-10 integer");
    }
    return value;
}
...
std::vector<int> parse_threads(const std::string& text) {
    ...
    while (begin <= text.size()) {
        ...
        if (end == begin) {
            throw std::invalid_argument("--threads-list contains an empty entry");
        }
        threads.push_back(parse_integer(text.substr(begin, end - begin), "--threads-list"));
...
std::string help_text() {
    return "Usage: csc3_demo_benchmark [options]\n"
...
std::string samples_csv_text(const BenchmarkResult& result) {
    ...
    for (const BenchmarkSample& sample : result.samples) {
        const CandidateTimings& timings = sample.candidate_timings;
        output << csv_escape(kBenchmarkSchemaVersion) << ',' << csv_escape(result.case_name) << ','
               << csv_escape(result.element_type) << ',' << result.configuration.nx << ','
               << result.configuration.ny << ',' << result.configuration.nz << ','
...
std::string summary_json_text(const BenchmarkResult& result) {
    ...
    output << std::setprecision(std::numeric_limits<double>::max_digits10);
    output << "{\n"
           << "  \"schema_version\": " << json_escape(kBenchmarkSchemaVersion) << ",\n"
           << "  \"configuration\": {\n"
           << "    \"case\": "
           ...
           << "    \"thread_counts\": [";
    for (std::size_t index = 0; index < result.configuration.thread_counts.size(); ++index) {
        if (index != 0) {
            output << ", ";
        }
    ...
    output << "],\n"
           << "    \"warmup_count\": " << result.configuration.warmup_count << ",\n"
           << "    \"repeat_count\": " << result.configuration.repeat_count << ",\n"
           << "    \"amortization_count\": " << result.configuration.amortization_count << ",\n"
           ...
           << "    \"relative_frobenius_error\": " << result.correctness.relative_frobenius_error
           << ",\n"
           << "    \"max_absolute_error\": " << result.correctness.max_absolute_error << ",\n"
           ...
           << "    \"relative_frobenius_error_max\": " << kRelativeFrobeniusTolerance << ",\n"
           << "    \"relative_displacement_error_max\": " << kRelativeDisplacementTolerance << ",\n"
           << "    \"relative_residual_max\": " << kRelativeResidualTolerance << '\n'
           << "  },\n"
           << "  \"validation_cases\": [";
    for (std::size_t index = 0; index < result.validation_cases.size(); ++index) {
        const ValidationResult& validation = result.validation_cases[index];
        output << (index == 0 ? "\n" : ",\n") << "    {\n"
               << "      \"case_name\": " << json_escape(validation.case_name) << ",\n"
               << "      \"element_type\": "
               ...
               << "        \"max_absolute_error\": " << validation.matrix.max_absolute_error
               << ",\n"
               << "        \"reference_max_absolute_value\": "
               << validation.matrix.reference_max_absolute_value << ",\n"
               << "        \"max_absolute_tolerance\": " << validation.matrix.max_absolute_tolerance
               << ",\n"
               << "        \"status\": " << json_escape(validation_status(validation.matrix.passed))
               << '\n'
               << "      },\n"
               << "      \"displacement\": {\n"
               ...
               << "        \"status\": "
               << json_escape(validation_status(validation.displacement.passed)) << '\n'
               << "      },\n"
    ...
    for (std::size_t index = 0; index < result.per_thread_measured.size(); ++index) {
        ...
        output << ",\n"
               << "      \"symbolic_speedup\": " << summary.symbolic_speedup << ",\n"
               << "      \"numeric_speedup\": " << summary.numeric_speedup << '\n'
               << "    }";
    }
    output << "\n  ],\n"
           << "  \"estimated_persistent_bytes\": " << result.estimated_persistent_bytes << ",\n"
           << "  \"estimated_persistent_memory_kind\": "
           << json_escape("owned_vector_payload_bytes_not_rss") << ",\n"
           << "  \"numeric_speedup_basis\": "
           << json_escape("serial_reset_plus_kernel_over_atomic_reset_plus_kernel") << ",\n"
           << "  \"performance_evidence_level\": " << json_escape(result.performance_evidence_level)
           << ",\n"
           << "  \"performance_gate\": {\n"
           << "    \"status\": " << json_escape(result.performance_gate.status) << ",\n"
           << "    \"applicable\": " << (result.performance_gate.applicable ? "true" : "false")
           << ",\n"
           << "    \"performance_requirements_met\": "
           << (result.performance_gate.performance_requirements_met ? "true" : "false") << ",\n"
           ...
           << "  },\n"
           << "  \"performance_gate_status\": " << json_escape(result.performance_gate_status)
           << '\n'
           << "}\n";
    return output.str();
}
...
int run_benchmark_cli(const std::vector<std::string>& arguments, std::ostream& standard_output,
    ...
    try {
        ...
        const auto option_value = [&arguments](std::size_t& index, const std::string& option) {
        ...
        for (std::size_t index = 0; index < arguments.size(); ++index) {
            ...
            if (option == "--dry-run") {
                ...
                if (value == "generated-tet4") {
                    ...
                    configuration.benchmark_case = BenchmarkCase::GeneratedHex8;
                } else if (value == "windhub") {
                    configuration.benchmark_case = BenchmarkCase::WindHub;
                } else {
                    throw std::invalid_argument("unsupported benchmark case: " + value);
                }
            } else if (option == "--nx") {
                mark_seen(option);
                configuration.nx = parse_integer(option_value(index, option), "--nx");
            } else if (option == "--ny") {
                mark_seen(option);
                configuration.ny = parse_integer(option_value(index, option), "--ny");
            } else if (option == "--nz") {
                mark_seen(option);
                configuration.nz = parse_integer(option_value(index, option), "--nz");
            } else if (option == "--threads-list") {
                mark_seen(option);
                configuration.thread_counts = parse_threads(option_value(index, option));
            } else if (option == "--warmup") {
                mark_seen(option);
                configuration.warmup_count = parse_integer(option_value(index, option), "--warmup");
            } else if (option == "--repeat") {
                mark_seen(option);
                configuration.repeat_count = parse_integer(option_value(index, option), "--repeat");
            } else if (option == "--amortization-count") {
                mark_seen(option);
                configuration.amortization_count =
                    parse_integer(option_value(index, option), "--amortization-count");
                ...
                if (value == "ci-smoke") {
                    ...
                    throw std::invalid_argument("unsupported performance evidence level: " + value);
                }
            } else if (option == "--input") {
                mark_seen(option);
                configuration.input_path = std::filesystem::u8path(option_value(index, option));
            } else if (option == "--samples-csv") {
                mark_seen(option);
                samples_path = std::filesystem::u8path(option_value(index, option));
            } else if (option == "--summary-json") {
                mark_seen(option);
                summary_path = std::filesystem::u8path(option_value(index, option));
            } else {
        ...
        if (configuration.benchmark_case == BenchmarkCase::WindHub) {
            if (seen_options.count("--nx") != 0 || seen_options.count("--ny") != 0 ||
