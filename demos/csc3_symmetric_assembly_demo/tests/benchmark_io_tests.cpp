...
ValidationResult synthetic_validation(ElementType element_type, int thread_count) {
    ValidationResult validation;
    validation.case_name =
        element_type == ElementType::Tet4 ? "cube_tet4_1x1x1" : "cube_hex8_1x1x1";
    ...
    validation.matrix = MatrixComparison{
        true, 1.0e-12, 2.0e-12, 0.99, 1.0e-8, true,
    };
    validation.displacement = DisplacementComparison{
        3.0e-12, 4.0e-12, 5.0e-12, 6.0e-6, 7.0e-6, true,
    };
...
BenchmarkResult synthetic_result() {
    ...
    result.configuration.performance_evidence_level = PerformanceEvidenceLevel::CiSmoke;
    ...
    summary.symbolic_speedup =
        result.serial_measured.symbolic_total_ms.median_ms / summary.symbolic_total_ms.median_ms;
    summary.numeric_speedup =
        result.serial_measured.numeric_total_ms.median_ms /
        (summary.numeric_reset_ms.median_ms + summary.numeric_kernel_ms.median_ms);
    ...
    warmup.amortized_total_ms = warmup.candidate_timings.symbolic_total_ms / 2.0 +
    ...
    measured.amortized_total_ms = measured.candidate_timings.symbolic_total_ms / 2.0 +
...
class JsonSyntaxValidator {
  ...
  private:
        ...
        while (position_ < text_.size()) {
            ...
            if (character == '\\') {
                ...
                if (escaped == 'u') {
                    ...
                    require_or_fail(escaped == '"' || escaped == '\\' || escaped == '/' ||
    ...
    void parse_literal(const char* literal) {
        while (*literal != '\0') {
            require_or_fail(position_ < text_.size() && text_[position_] == *literal);
            ++position_;
            ++literal;
        }
    }
...
void test_csv_schema_escaping_and_round_trip_numbers() {
    ...
    for (std::size_t row = 1; row < records.size(); ++row) {
        ...
        for (const std::size_t field : numeric_fields) {
            ...
            require_true(consumed == records[row][field].size() && std::isfinite(parsed),
...
void test_json_is_valid_complete_utf8_without_fabricated_provenance() {
    ...
    const std::string reference_scale_key = "\"reference_max_absolute_value\"";
    std::size_t reference_scale_count = 0;
    for (std::size_t position = json.find(reference_scale_key); position != std::string::npos;
         position = json.find(reference_scale_key, position + reference_scale_key.size())) {
...
void test_invalid_result_is_rejected_before_serialization() {
    ...
    require_throws<std::runtime_error>([&result] { static_cast<void>(samples_csv_text(result)); },
                                       "failed correctness CSV");
    require_throws<std::runtime_error>([&result] { static_cast<void>(summary_json_text(result)); },
...
void test_reference_scaled_tolerances_are_bound_before_serialization() {
    ...
    const auto exercise = [&require_rejected](const std::string& label, const auto& select_matrix) {
        BenchmarkResult result = synthetic_result();
    ...
    exercise("root",
             [](BenchmarkResult& result) -> BenchmarkCorrectness& { return result.correctness; });
...
void test_recomputed_evidence_rejects_summary_and_raw_tampering() {
    ...
    const std::vector<PhaseField> phase_fields{
        &ThreadBenchmarkSummary::symbolic_pattern_ms, &ThreadBenchmarkSummary::symbolic_scatter_ms,
        &ThreadBenchmarkSummary::symbolic_total_ms,   &ThreadBenchmarkSummary::numeric_reset_ms,
        &ThreadBenchmarkSummary::numeric_kernel_ms,   &ThreadBenchmarkSummary::numeric_algorithm_ms,
        &ThreadBenchmarkSummary::numeric_total_ms,    &ThreadBenchmarkSummary::amortized_total_ms,
    };
...

    {
        ...
        result.samples[0].symbolic_speedup = result.per_thread_measured.front().symbolic_speedup;
        result.samples[1].symbolic_speedup = result.per_thread_measured.front().symbolic_speedup;
...
void test_help_version_and_deterministic_dry_run() {
    ...
    output.str("");
    output.clear();
    require_equal(run_benchmark_cli({"--version"}, output, error), 0, "version exit code");
    require_equal(output.str(), std::string("csc3_demo_benchmark 0.2.0\n"), "version output");
    ...
    error.str("");
    error.clear();
    require_equal(run_benchmark_cli(arguments, output, error), 0, "dry-run exit code");
    const std::string first_plan = output.str();
    output.str("");
    output.clear();
    require_equal(run_benchmark_cli(arguments, output, error), 0, "second dry-run exit code");
...
void test_invalid_arguments_and_output_contracts() {
    ...
    for (const std::vector<std::string>& arguments : {
             ...
             {"--threads-list", "1", "--warmup", "0", "--repeat", "1", "--samples-csv", csv_text},
             {"--threads-list", "1", "--warmup", "0", "--repeat", "1", "--samples-csv", csv_text,
              "--summary-json", csv_text},
         }) {
    ...
    const std::filesystem::path missing_parent = temporary.path() / "missing";
    static_cast<void>(run_invalid({
        "--threads-list",
        "1",
        "--warmup",
        ...
        "1",
        "--samples-csv",
        (missing_parent / "samples.csv").string(),
        "--summary-json",
        (missing_parent / "summary.json").string(),
    }));
    require_true(!std::filesystem::exists(missing_parent), "CLI created a missing output parent");
...
void test_normal_cli_writes_both_outputs_and_refuses_overwrite() {
    ...
    std::ostringstream output;
    std::ostringstream error;
    require_equal(run_benchmark_cli(arguments, output, error), 0, "normal CLI exit code");
...
void test_direct_file_writers_refuse_existing_paths() {
    ...
    require_equal(read_file(existing_csv), std::string("sentinel-csv"),
                  "CSV writer overwrite refusal");
    require_equal(read_file(existing_json), std::string("sentinel-json"),
...
void test_direct_file_writers_refuse_dangling_symlinks() {
...
#endif
    require_true(!csv_error && !json_error, "could not create dangling output symlinks");
    ...
    require_true(std::filesystem::is_symlink(std::filesystem::symlink_status(csv_link)) &&
                     std::filesystem::is_symlink(std::filesystem::symlink_status(json_link)),
                 "writer replaced a dangling output symlink");
    require_true(!std::filesystem::exists(missing_csv) && !std::filesystem::exists(missing_json),
...
void test_zero_serial_baseline_is_reported_as_zero_speedup() {
    ...
    const std::string csv = samples_csv_text(result);
    const std::string json = summary_json_text(result);
    require_true(!csv.empty() && !json.empty(), "zero serial baseline could not be serialized");
}
