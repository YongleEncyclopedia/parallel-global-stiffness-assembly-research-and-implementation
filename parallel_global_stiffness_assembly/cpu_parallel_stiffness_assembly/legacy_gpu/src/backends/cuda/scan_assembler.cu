/**
 * @file scan_assembler.cu
 * @brief 前缀和分配组装器 CUDA 实现
 */
...
/**
 * @brief 前缀和组装 Kernel（简化实现，与原子版本类似）
 *
 * 完整实现应包含：
 * 1. 计数 kernel
 * 2. 前缀和计算（Thrust）
 * 3. 填值 kernel
 */
__global__ void assemble_hex8_scan_kernel(
...
__global__ void assemble_tet4_scan_kernel(
    const double* __restrict__ node_x,
    const double* __restrict__ node_y,
    const double* __restrict__ node_z,
    const int* __restrict__ connectivity,
...
ScanAssembler::ScanAssembler(const AssemblyOptions& options)
    : options_(options) {}
...
void ScanAssembler::set_mesh(const Mesh& mesh) {
    mesh_ = &mesh;
}

void ScanAssembler::set_csr_structure(const CsrMatrix& csr) {
    host_result_ = csr;
}

void ScanAssembler::prepare() {
...
void ScanAssembler::assemble() {
    ...
    if (mesh_->element_type == ElementType::Hex8) {
        cuda::assemble_hex8_scan_kernel<<<grid_size, block_size>>>(
        ...
        cuda::assemble_tet4_scan_kernel<<<grid_size, block_size>>>(
            d_nodes_.d_x, d_nodes_.d_y, d_nodes_.d_z,
            d_conn_.d_data,
            d_csr_.d_row_ptr, d_csr_.d_col_ind, d_csr_.d_values,
            num_elements
