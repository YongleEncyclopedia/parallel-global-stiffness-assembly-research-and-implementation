...
\end{frame}
\note{
开场时先说明这不是重新介绍整个项目，而是针对 mentor 最近给出的两个方向做一次阶段性整理。标题里的“二阶段组装”指符号组装和数值组装分开；“硬件适配”指线程数、物理核、P/E core profile 和 OpenMP 绑定设置这些因素。这里不需要展开细节，只要告诉听众：后面会用已有实验数据回答为什么要这样拆、拆开以后有什么收益。
}

\begin{frame}{这次先把 mentor 的问题拆成两条证据链}
...
\begin{itemize}
  \item 线程数增加不等于一定加速。
  \item P-core / E-core 或 QoS profile 改变了可用资源。
  \item Apple 和 Intel 的隔离机制不能混成同一种结论。
\end{itemize}
\end{column}
\end{columns}
\end{frame}
\note{
这一页的目的，是把 mentor 的四个问题收敛成两条主线。左边讲效率：单次和多次组装为什么不同，因为符号阶段如果能复用，多次组装时预处理成本会被摊薄。右边讲硬件：同一个并行算法在不同 core profile 上表现不同，所以不能只给一个线程数加速曲线。口头过渡可以说：我先讲左边，也就是符号/数值拆分到底做了什么，再讲右边的硬件适配。
}
...
\begin{frame}{效率比较只用一个固定算例}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.47\textwidth}
\textbf{WindTurbineHub}
\begin{itemize}
...
\end{columns}
\begin{block}{这样做的原因}
固定网格、单元类型、核函数和平台后，表里的差异才主要反映符号结构复用带来的收益。
\end{block}
\end{frame}
\note{
这一页要解释实验设置。WindTurbineHub 是真实工程网格，规模足够大，能体现稀疏结构构建和全局写入的成本。nodes 是节点数，elements 是单元数，dofs 是自由度数；这里每个节点 3 个位移自由度，所以 DOF 数约等于节点数乘以 3。kernel 选 \texttt{physics\_tet4}，是因为这组比较要回答真实 Tet4 物理组装成本，而不是只看 synthetic assembly overhead。右边平台信息很重要：这张符号/无符号表是在 Apple M4 Max 上跑的，不应该直接拿去和 Intel 时间做绝对比较。
}
...
\end{frame}
\note{
这页是核心结果，讲慢一点。第一列是同一个符号结构下做几次数值组装。符号总耗时包括 CSR 构建和 scatter plan 构建。数值/次是每次 \texttt{physics\_tet4} 组装的平均时间。符号复用摊销总耗时是把一次符号成本加上多次数值成本后除以组装次数。右边三列是无符号直接组装：先生成所有 row、col、value 贡献，再排序归并。最后一列收益是无符号直接总耗时除以符号复用摊销总耗时。要强调两个数字：一次组装已经有 1.644x，说明即使只组装一次，排序归并也很贵；30 次达到 8.086x，说明多次迭代场景下符号复用特别有价值。
}
...
\begin{itemize}
  ...
  \item 同时记录 timing、speedup、efficiency、extra memory。
\end{itemize}
\end{column}
\begin{column}{0.46\textwidth}
\textbf{核心资源 profile}
\begin{itemize}
  ...
  \item 不把调度机制差异写成算法本质差异。
\end{itemize}
\end{column}
\end{columns}
\end{frame}
\note{
这一页进入硬件适配。先讲 OpenMP 环境：default 是让运行时自己调度，bound 是显式关闭动态线程并尽量绑定到 cores。这样可以看线程绑定对性能有没有影响。右边讲核心 profile：Intel 的 taskset 是比较硬的 CPU affinity 限制，可以把进程限制在 P-core 或 E-core 的 CPU id 上；Apple Silicon 没有同样公开的 per-core 绑定接口，所以这里用 QoS 或 taskpolicy 做偏置实验。这里一定要讲边界：Apple 的结果叫 QoS-biased sensitivity，不是严格 E-core-only 或 P-core-only。
}
...
\begin{itemize}
  ...
  \item P/E profile：Intel 与 Apple 的实验边界已经分开记录。
...
\end{frame}
\note{
这一页适合在进入结论前停一下，把“现在能说什么”和“不能过度说什么”分开。左边都是已有数据可以支撑的结论：符号复用有收益，无符号直接路径主要慢在排序归并，Intel 和 Apple 的 profile 边界已经记录，当前平台没有 SMT 逻辑核收益证据。右边是下一步：第一，符号阶段本身现在仍有 CSR 和 scatter 构建成本，可以考虑并行化；第二，研究符号 artifact 的生命周期，看能否和数值组装解耦甚至流水；第三，找一台有 SMT 的 CPU，才能真正回答物理核和逻辑核的区别。
}
...
\begin{enumerate}
  ...
  \item \keyword{硬件适配要带实验边界}：Intel 的 \texttt{taskset} 与 Apple 的 QoS profile 不是同一种隔离机制；当前没有 SMT 逻辑核收益证据。
\end{enumerate}
\end{frame}
\note{
结论页建议按四句话讲。第一句：这个项目不是停留在概念上，符号阶段和数值阶段在 C++ 里已经有对应实现。第二句：最重要的数字是 1.644x 到 8.086x，说明单次有收益，多次更明显。第三句：无符号直接组装慢的主要原因不是 Ke 计算，而是贡献生成后的排序归并和临时内存。第四句：硬件适配不能只看一条加速曲线，必须说明 P/E profile、QoS、物理核和逻辑核边界。最后可以自然过渡到导师下一步建议：接下来我会优先补符号组装并行化和真正的逻辑核平台实验。
}
...
\end{frame}
\note{
这页是 Intel P-core-only 的附录图。只有在 mentor 追问 Intel P-core 限制下每个算法怎么变时再展开。讲法是：这个 profile 使用 \texttt{taskset -c 0-7}，把运行限制在 CPU capacity 更高的一组核心上；图里展示 bound 环境下各算法随线程数变化的时间、加速比、效率和内存。这里不是和 Apple 直接比，而是和 Intel \texttt{full\_host}、Intel E-core-only 做同平台对照。
}
...
\end{frame}
\note{
这页是 Intel E-core-only。它使用 taskset -c 8-19，把进程限制在 CPU capacity 较低的一组核心上。这个图的用途是解释：E-core 数量更多不一定带来更好的组装时间，因为单核能力、内存系统、写冲突和算法负载均衡都会影响最终结果。如果 mentor 问 P-core 和 E-core 差异，可以把这页和上一页一起讲。
}
