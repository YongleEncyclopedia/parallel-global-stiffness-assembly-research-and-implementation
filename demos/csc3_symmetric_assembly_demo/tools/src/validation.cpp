...
ValidatedCase validate_reference_input(const AssemblyCase& assembly_case) {
    ...
    const std::size_t expected_offset_count = checked_add(element_count, 1, "element offset count");
    ...
    std::iota(validated.canonical_ordinals.begin(), validated.canonical_ordinals.end(),
              std::size_t{0});
    ...
    std::sort(validated.canonical_ordinals.begin(), validated.canonical_ordinals.end(),
              [&topology](std::size_t left, std::size_t right) {
                  return topology.element_ids[left] < topology.element_ids[right];
              });
    for (std::size_t index = 1; index < validated.canonical_ordinals.size(); ++index) {
    ...
    for (std::size_t index = 1; index < matrices.element_value_offsets.size(); ++index) {
        if (matrices.element_value_offsets[index] < matrices.element_value_offsets[index - 1]) {
            throw std::invalid_argument("element_value_offsets must be monotone");
        }
    }
    if (matrices.element_value_offsets.back() !=
        size_to_offset(matrices.values_row_major.size(), "element matrix value array size")) {
        throw std::invalid_argument(
    ...
    for (std::size_t canonical = 0; canonical < element_count; ++canonical) {
        ...
        const std::size_t local_dimension = input_local_dimensions[input_ordinal];
        ...
        const Offset segment_size = matrices.element_value_offsets[canonical + 1] -
                                    matrices.element_value_offsets[canonical];
        if (offset_to_size(segment_size, "local matrix size") != expected_matrix_size) {
            throw std::invalid_argument(
                "each matrix segment must contain local_dimension squared values");
        }
        const std::size_t matrix_begin = offset_to_size(matrices.element_value_offsets[canonical],
        ...
        for (std::size_t row = 0; row < local_dimension; ++row) {
            for (std::size_t column = 0; column < local_dimension; ++column) {
                const double value =
                    matrices.values_row_major[matrix_begin + row * local_dimension + column];
        ...
        for (std::size_t row = 0; row < local_dimension; ++row) {
            for (std::size_t column = row + 1; column < local_dimension; ++column) {
                const double upper =
                    matrices.values_row_major[matrix_begin + row * local_dimension + column];
                const double lower =
                    matrices.values_row_major[matrix_begin + column * local_dimension + row];
...
void validate_reference_result(const SerialAssemblyResult& reference) {
    ...
    if (reference.column_offsets.front() != 0 ||
        reference.column_offsets.back() !=
            size_to_offset(reference.row_indices.size(), "reference row index count")) {
        throw std::invalid_argument("reference column_offsets is inconsistent");
    }
    for (std::size_t column = 0; column < dimension; ++column) {
        const std::size_t begin =
            offset_to_size(reference.column_offsets[column], "reference column offset");
        const std::size_t end =
            offset_to_size(reference.column_offsets[column + 1], "reference column offset");
    ...
    if (!std::all_of(reference.dense_values.begin(), reference.dense_values.end(),
...
bool validate_candidate_structure(const Csc3Matrix& candidate) {
    ...
    return std::all_of(candidate.values.begin(), candidate.values.end(),
                       [](double value) { return std::isfinite(value); });
}
  ...
  public:
    ...
    void add(double value) {
        ...
        if (scale_ < magnitude) {
            ...
            scaled_square_sum_ = 1.0 + scaled_square_sum_ * ratio * ratio;
            scale_ = magnitude;
        } else {
    ...
    [[nodiscard]] double relative_to(const ScaledNorm& reference, double reference_floor) const {
        ...
        return (scale_ / reference.scale_) *
               std::sqrt(scaled_square_sum_ / reference.scaled_square_sum_);
    }
...
std::vector<double> solve_dense_system(std::vector<double> matrix,
    ...
    if (!std::all_of(matrix.begin(), matrix.end(),
                     [](double value) { return std::isfinite(value); }) ||
        !std::all_of(right_hand_side.begin(), right_hand_side.end(),
                     [](double value) { return std::isfinite(value); })) {
    ...
    for (std::size_t column = 0; column < dimension; ++column) {
        ...
        if (!std::isfinite(pivot_magnitude) || pivot_magnitude <= pivot_tolerance) {
            throw std::runtime_error("free stiffness matrix is singular");
        }
        if (pivot_row != column) {
...
FreeSystem extract_free_system(const std::vector<double>& dense_matrix,
    ...
    if (dense_matrix.size() != checked_multiply(dimension, dimension, "global dense matrix size")) {
        throw std::invalid_argument("dense matrix size must equal global_dimension squared");
    }
    if (force.size() != dimension) {
        throw std::invalid_argument("force size must equal the global dimension");
    }
    if (!std::all_of(force.begin(), force.end(),
                     [](double value) { return std::isfinite(value); })) {
        throw std::invalid_argument("force must contain only finite values");
    }
    if (!std::is_sorted(constraints.begin(), constraints.end()) ||
        std::adjacent_find(constraints.begin(), constraints.end()) != constraints.end()) {
    ...
    result.matrix.assign(checked_multiply(free_dimension, free_dimension, "free matrix size"), 0.0);
    ...
    for (std::size_t free_row = 0; free_row < free_dimension; ++free_row) {
        ...
        for (std::size_t free_column = 0; free_column < free_dimension; ++free_column) {
...
std::vector<double> reconstruct_displacement(std::size_t global_dimension,
...
double relative_residual(const FreeSystem& system, const std::vector<double>& displacement) {
    ...
    for (std::size_t row = 0; row < dimension; ++row) {
        ...
        for (std::size_t column = 0; column < dimension; ++column) {
            value += system.matrix[row * dimension + column] * displacement[column];
        }
        residual[row] = value;
    }
...
SerialAssemblyResult assemble_serial_reference(const AssemblyCase& assembly_case) {
    ...
    for (std::size_t canonical = 0; canonical < validated.canonical_ordinals.size(); ++canonical) {
        ...
        const std::size_t dof_begin =
            static_cast<std::size_t>(topology.element_dof_offsets[input_ordinal]);
        const std::size_t local_dimension = validated.local_dimensions[canonical];
        const std::size_t matrix_begin =
            static_cast<std::size_t>(matrices.element_value_offsets[canonical]);
        for (std::size_t local_row = 0; local_row < local_dimension; ++local_row) {
            const GlobalDofIndex global_row = topology.global_dof_indices[dof_begin + local_row];
            for (std::size_t local_column = 0; local_column < local_dimension; ++local_column) {
                const GlobalDofIndex global_column =
                    topology.global_dof_indices[dof_begin + local_column];
                const double local_value =
                    matrices.values_row_major[matrix_begin + local_row * local_dimension +
                                              local_column];
                double& global_value =
                    result.dense_values[static_cast<std::size_t>(global_row) * dimension +
                                        static_cast<std::size_t>(global_column)];
                global_value += local_value;
                if (!std::isfinite(global_value)) {
                    throw std::overflow_error("serial reference accumulation became nonfinite");
                }
...
MatrixComparison compare_matrices(const Csc3Matrix& candidate,
    ...
    for (std::size_t index = 0; index < reference.dense_values.size(); ++index) {
        const double difference = candidate_dense[index] - reference.dense_values[index];
...
ValidationResult validate_case(const AssemblyCase& assembly_case, int thread_count) {
    ...
    if (!std::all_of(assembly_case.force.begin(), assembly_case.force.end(),
                     [](double value) { return std::isfinite(value); })) {
        throw std::invalid_argument("force must contain only finite values");
    }
    ...
    const FreeSystem reference_free =
        extract_free_system(reference.dense_values, reference.dimension, assembly_case.force,
                            assembly_case.constrained_dof_indices);
    ...
    const std::vector<double> candidate_displacement =
        reconstruct_displacement(dimension, candidate_free, candidate_free_displacement);
    const std::vector<double> reference_displacement =
        reconstruct_displacement(dimension, reference_free, reference_free_displacement);

    result.displacement.relative_displacement_error =
        relative_displacement_error(candidate_displacement, reference_displacement);
    ...
    result.displacement.parallel_displacement_norm = vector_norm(candidate_displacement);
    result.displacement.serial_displacement_norm = vector_norm(reference_displacement);
    result.displacement.passed =
        ...
        result.displacement.relative_displacement_error <= kRelativeDisplacementTolerance &&
        result.displacement.parallel_relative_residual <= kRelativeResidualTolerance &&
        result.displacement.serial_relative_residual <= kRelativeResidualTolerance;
